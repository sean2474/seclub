  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* 텍스트 영역 */}
        {(currentPopup.title || currentPopup.content || hasLink) && (
          <div className={hasImage ? "px-6 py-5" : "px-6 pt-8 pb-6"}>
            ...
            {currentPopup.content && (
              <p className={`${currentPopup.title ? "mt-2.5 " : ""}text-sm text-gray-500 leading-relaxed`}>
                {currentPopup.content}
              </p>
            )}
            {/* 링크 CTA 버튼 */}
            {hasLink && (
              <button
                className={`${currentPopup.title || currentPopup.content ? "mt-4 " : ""}inline-flex items-center gap-1 text-sm text-gray-900 font-medium tracking-tight hover:text-gray-600 transition-colors`}
